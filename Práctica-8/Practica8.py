...
for n in (16000, 32000, 64000, 128000):
    ...
    for paso in range(duracion):
        ...
        PR=((restantes*100)/cantidad)
        porcentaje.append(PR)
        itera.append(paso)
        grandes.clear()

primero= porcentaje[0:50]
segundo= porcentaje[50:100]
tercero= porcentaje[100:150]
cuarto= porcentaje[150:200]
P1=[]
S2=[]
T3=[]
C4=[]
...
for F in range(duracion):
    P1.append([primero[F],F])
    S2.append([segundo[F],F])
    T3.append([tercero[F],F])
    C4.append([cuarto[F],F])
...
plt.ylim([25, 40])
plt.scatter(Pvert, prim[0], color='black')
plt.scatter(Svert, seg[0], color='black')
plt.plot(itera, porcentaje[0:50],color='red', label='16k')
plt.plot(itera, porcentaje[50:100],color='blue', label='32k')
plt.axvline(x = Pvert, color = 'red')
plt.axvline(x = Svert, color = 'blue')
plt.xlabel('iteracion que se filtra')
plt.ylabel('% filtrado')
plt.legend()
plt.legend(loc='lower right', title='Cambio respecto al porcentaje')
plt.show()
plt.close()


plt.ylim([25, 40])
plt.scatter(Tvert, ter[0], color='black')
plt.scatter(Cvert, cuar[0], color='black')
plt.plot(itera, porcentaje[100:150], color='orange', label='64k')
plt.plot(itera, porcentaje[150:200], color='green', label='128k')
plt.axvline(x = Tvert, color = 'orange')
plt.axvline(x = Cvert, color = 'green')
plt.xlabel('iteracion que se filtra')
plt.ylabel('% filtrado')
plt.legend()
plt.legend(loc='lower right', title='Cambio respecto al porcentaje')
